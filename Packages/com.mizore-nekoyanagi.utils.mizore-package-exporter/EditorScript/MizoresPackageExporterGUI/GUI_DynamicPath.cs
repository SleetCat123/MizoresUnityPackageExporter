            ...
            if ( CustomFoldout.EditorPrefFoldout(
                    ...
                    onRightClick = ( ) => {
                        ...
                        menu.AddItem( new GUIContent( ExporterTexts.ConvertAllPathsToAbsolute ), false, ( ) => {
                            foreach ( var item in targetlist ) {
                                for ( int i = 0; i < item.dynamicpath.Count; i++ ) {
                                    ...
                                    item.dynamicpath[i].path = PathUtils.GetProjectAbsolutePath( dir, item.dynamicpath[i].path );
                                }
                                EditorUtility.SetDirty( item );
                            }
                        } );
                        menu.AddItem( new GUIContent( ExporterTexts.ConvertAllPathsToRelative ), false, ( ) => {
                            foreach ( var item in targetlist ) {
                                for ( int i = 0; i < item.dynamicpath.Count; i++ ) {
                                    var dir = item.GetDirectoryPath( );
                                    item.dynamicpath[i].path = PathUtils.GetRelativePath( dir, item.dynamicpath[i].path );
                                }
                                EditorUtility.SetDirty( item );
                            }
                        } );
                        GUIElement_CopyPasteList.OnRightClickFoldout( targetlist, ExporterTexts.FoldoutDynamicPath, ( ex ) => ex.dynamicpath, ( ex, list ) => ex.dynamicpath = list, menu );
                    }
                }
                ) ) {
                ...
                for ( int i = 0; i < dpath_count.max; i++ ) {
                    using ( var horizontalScope = new EditorGUILayout.HorizontalScope( ) ) {
                        ...
                        if ( multiple ) {
                            samevalue_in_all = i < dpath_count.min && targetlist.All( v => t.dynamicpath[i].path == v.dynamicpath[i].path );
                        }
                        ...
                        if ( samevalue_in_all ) {
                            ...
                            if ( currentEvent.type == EventType.ContextClick && rect.Contains( currentEvent.mousePosition ) ) {
                                ...
                                int index= i;
                                if ( PathUtils.IsRelativePath( t.dynamicpath[i].path ) ) {
                                    menu.AddItem( new GUIContent( ExporterTexts.ConvertToAbsolutePath ), false, ( ) => {
                                        foreach ( var item in targetlist ) {
                                            if ( index < item.dynamicpath.Count ) {
                                                var dir = item.GetDirectoryPath( );
                                                item.dynamicpath[index].path = PathUtils.GetProjectAbsolutePath( dir, item.dynamicpath[index].path );
                                                EditorUtility.SetDirty( item );
                                            }
                                        }
                                    ...
                                    menu.AddItem( new GUIContent( ExporterTexts.ConvertToRelativePath ), false, ( ) => {
                                        foreach ( var item in targetlist ) {
                                            if ( index < item.dynamicpath.Count ) {
                                                var dir = item.GetDirectoryPath( );
                                                item.dynamicpath[index].path = PathUtils.GetRelativePath( dir, item.dynamicpath[index].path );
                                                EditorUtility.SetDirty( item );
                                            }
                                        }
                                        GUI.FocusControl( null );
                        ...
                        string path;
                        if ( samevalue_in_all ) {
                            path = EditorGUI.TextField( textrect, t.dynamicpath[i].path );
                        } else {
                        ...
                        if ( EditorGUI.EndChangeCheck( ) ) {
                            ...
                            foreach ( var item in targetlist ) {
                                ExporterUtils.ResizeList( item.dynamicpath, Mathf.Max( i + 1, item.dynamicpath.Count ), ( ) => new DynamicPathElement( ) );
                                item.dynamicpath[i].path = path;
                                EditorUtility.SetDirty( item );
                            }
                    ...
                    using ( new EditorGUI.DisabledScope( !useReferences ) ) {
                        ...
                        using ( var horizontalScope = new EditorGUILayout.HorizontalScope( ) ) {
                            ...
                            if ( multiple ) {
                                samevalue_searchReference = i < dpath_count.min && targetlist.All( v => t.dynamicpath[i].searchReference == v.dynamicpath[i].searchReference );
                            }
                            EditorGUI.BeginChangeCheck( );
                            ...
                            bool searchReference = EditorGUILayout.Toggle(content, t.dynamicpath[i].searchReference );
                            EditorGUI.showMixedValue = false;
                            if ( EditorGUI.EndChangeCheck( ) ) {
                                foreach ( var item in targetlist ) {
                                    ExporterUtils.ResizeList( item.dynamicpath, Mathf.Max( i + 1, item.dynamicpath.Count ), ( ) => new DynamicPathElement( ) );
                                    item.dynamicpath[i].searchReference = searchReference;
                                    EditorUtility.SetDirty( item );
                                }
                                dpath_count = MinMax.Create( targetlist, v => v.dynamicpath.Count );
                            }
                ...
                if ( GUIElement_Utils.PlusButton( ) ) {
                    foreach ( var item in targetlist ) {
                        ExporterUtils.ResizeList( item.dynamicpath, dpath_count.max + 1, ( ) => new DynamicPathElement( ) );
                        EditorUtility.SetDirty( item );
                    }
                }
                EditorGUI.indentLevel--;
                        ...
                        for ( int i = 0; i < dpath_count.max; i++ ) {
                            using ( var horizontalScope = new EditorGUILayout.HorizontalScope( ) ) {
                                ...
                                if ( i < item.dynamicpath.Count ) {
                                    string previewpath = item.ConvertDynamicPath( item.dynamicpath[i].path, true );
