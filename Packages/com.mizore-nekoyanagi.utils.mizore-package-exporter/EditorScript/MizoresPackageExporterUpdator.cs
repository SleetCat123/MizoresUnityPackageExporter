...
#pragma warning disable 612
            ...
            if ( latest != null ) {
                // V2オブジェクトのバージョン番号を最新に更新
                latest.packageExporterVersion = MizoresPackageExporter.CURRENT_PACKAGE_EXPORTER_OBJECT_VERSION;
                EditorUtility.SetDirty( latest );
                return latest;
            }
            ...
            if ( latest != null && latest != obj ) {
                ...
                if ( File.Exists( bakPath ) ) {
                    bakPath = AssetDatabase.GenerateUniqueAssetPath( bakPath );
                }
