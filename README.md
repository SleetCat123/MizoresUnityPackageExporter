...
### ◇フォルダ整理（Organize in Folder）
エクスポートされたunitypackageを同名のフォルダに格納します。
追加コピーパスと組み合わせることで、Readmeや利用規約などのファイルをunitypackageと同じフォルダにまとめることができます。

フォルダ名はデフォルトではパッケージ名が使用されますが、任意の名前に変更することも可能です。
%で囲まれた文字列はDynamicPathVariablesの文字列に置き換えられます。
***

### ◇追加コピーパス（Additional Copy Paths）
フォルダ整理が有効な場合に使用できます。
指定したファイルやフォルダを、整理先フォルダにコピーします。
Readmeや利用規約、サムネイル画像などをunitypackageと一緒にまとめたい場合に便利です。
（MizoresPackageExporterを複数選択している場合、追加コピーパスの編集はできません）

各項目には以下の設定があります。
- コピー元（Source）：コピーするファイルまたはフォルダのパスを指定します。パスにはDynamicPathVariablesの変数を使用できます。
- 出力先名（Destination Name）：コピー先での名前を指定します。空の場合は元の名前が使用されます。変数置き換えが可能です。

出力先名の指定方法によってコピーの挙動が変わります。
- 空欄：元のファイル名／フォルダ名をそのまま使用
- ファイル名を指定（例：`readme.txt`）：指定した名前にリネームしてコピー
- `/`で終わる文字列を指定（例：`docs/`）：指定したサブフォルダ内に元のファイル名でコピー

フォルダをコピー元に指定した場合、フォルダ内の構造を維持したままコピーされます（.metaファイルは除外されます）。
***

### ◇zip作成（Create ZIP）
エクスポート後にzipファイルを作成する機能です。
Unity 2022.1以降が必要です。

- 圧縮レベル：`Optimal`（最高圧縮率）、`Fastest`（最速）、`NoCompression`（圧縮なし）から選択できます。デフォルトは`Optimal`です。
- zipフォルダ名：zip出力先のサブフォルダ名を指定できます。デフォルトは`_zip`です。空にするとパッケージと同じ場所に出力されます。

フォルダ整理の設定によってzipに含まれる内容が変わります。
- フォルダ整理ON：整理先フォルダの内容がまとめてzip化されます。unitypackageに加え、追加コピーパスで指定したファイル／フォルダもzipに含まれます。
- フォルダ整理OFF：unitypackage単体のみがzip化されます。追加コピーパスで指定したファイルはzipに含まれません。

Readmeや利用規約などのファイルをzipに同梱したい場合は、フォルダ整理をONにした上で追加コピーパスにファイルを設定してください。
***
